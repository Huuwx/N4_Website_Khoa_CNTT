...
const Header: React.FC<HeaderProps> = () => {
  ...
  // Fetch categories and transform them into navigation items
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        // Transform categories into navigation items
        const navigationItems: NavigationItem[] = categories.map(category => ({
          name: category.name,
          href: category.pageUrl,
          submenu: category.categoryGroups.map(group => ({
            name: group.name,
            href: group.pageUrl
          }))
        }));
  ...
  const [activeNavItem, setActiveNavItem] = useState<NavigationItem | undefined>(undefined);

  // Update active nav item when location or navigation changes
  useEffect(() => {
  ...
  return (
    <header className="font-sans bg-white shadow-md z-50 sticky top-0 relative">
      {/* Top Bar */}
        ...
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-1 ml-auto">
            {isAuthenticated ? (
              <>
                <div
                  className={`flex items-center px-2 sm:px-4 py-2 font-medium rounded transition-colors ${
                    userRole === "admin"
                      ? "opacity-50 cursor-not-allowed"
                      : "hover:bg-white/10 cursor-pointer"
                  }`}
                  onClick={() => userRole !== "admin" && setIsModalOpen(true)}
                  aria-label={userRole === "admin" ? "Profile not available for admin" : "View profile"}
                  title={userRole === "admin" ? "Profile not available for admin" : "View profile"}
                >
                  <img
      ...
      <div className="container mx-16 max-w-[calc(100%-8rem)]">
              ...
              navigation.map((item) => (
                <div key={item.name} className="relative group">
                  ...
                  {item.submenu && item.submenu.length > 0 && (
                    <div className="absolute left-0 mt-0 w-56 bg-white shadow-lg rounded-b-md invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-200 z-50 max-h-[calc(100vh-100%)] overflow-y-auto">
          ...
          >
            ...
            <nav className="py-2">
              {isLoading ? (
                ...
                <div className="text-gray-500 text-center py-4">No navigation items available</div>
              ) : (
                navigation.map((item) => (
